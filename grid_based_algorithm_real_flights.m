function [minimumPath, minimumAngles, waypoints] = ...
    grid_based_algorithm_real_flights(version, typeCost)

    % Cost function: (0) angles, (1) energy
    % Version: (0) normal, (1) optimized, (2) original
    
    load('E_model_simple.mat');
    addpath('experiments/scenarios');

    [area, obstPoints, ~] = waypoints2meters('real_flight_area.waypoints','real_flight_obstacles.waypoints');
    ...
    % folder: data/mat_real_exp/normal/info_xx_cells.mat or 
    %         data/mat_real_exp/optimized/info_xx_cells.mat
    if version == 0
        parentFolder = 'data/mat_real_exp/normal';
    elseif version == 1
        %parentFolder = 'data/mat_real_exp/optimized';
        parentFolder = 'data/mat_real_exp/test';
    else
        parentFolder = 'data/mat_real_exp/original';
    end

    % saving .mat file with all the data
    % folder: mat/info_xx_cells.mat
